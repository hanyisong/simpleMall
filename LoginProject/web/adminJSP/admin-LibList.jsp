                ...
                if (data) {
                    ...
                    for (var i = 0; i < list.length; i++) {
                        str += "<tr><td>"
                            ...
                            + "</td><td><a style='cursor:pointer;border: #5a6268;' class='btn' onclick = 'deleteLib("
                            + list[i].lib_id
                            + ")'>删除</a></td></tr>";
                    }
                    $("#table").append(str);
    ...
    function deleteLib(lib_id) {
        $.ajax({
            ...
            success: function (data) {
                if (data.flag) {
                    alert("删除成功！");
                    window.location.href = "admin-LibList.jsp";//重新执行自动查询
                } else {
                    alert("删除失败！");
                    window.location.href = "admin-LibList.jsp";//重新执行自动查询
                }
